#import "/formatting.typ": *
#import "/shared-data.typ": *

= Agua de Jamaica

== Equipment

- Pot
- 4 L. (1 gal.) fine-mesh grease keeper#Footnotes.grease-keeper

== Ingredients

- 1 liter of water per iteration (see Steps below)

- 70 grams (1 cup) hibiscus petals

- 1 cinnamon stick

#let allspice = footnote[Allspice has the taste of cinnamon, cloves, and nutmeg---it
  doesn't add anything _new_ here.]
- 1 pinch each of ground cloves, ground nutmeg, and ground allspice (optional#allspice)

#let piloncillo = footnote[Substitute 1:1 with brown sugar.]
- 90 grams (1/2 cup) piloncillo#piloncillo

- 200 grams (1 cup) white sugar

== Steps <jamaica-steps>

=== Phase 1: The Boiling

+ In a large pot, bring 1 liter of water to a boil.

+ Setting aside the white sugar, stir all ingredients into pot. Reduce temperature &
  simmer for 20-30 minutes.

+ Strain ingredients into container. *Do NOT throw ingredients away yet.*

+ Repeat previous steps with fresh water & the same ingredients until container is nearly
  full or leaves are no longer potent.#footnote[This repetition makes the jamaica
    incredibly strong; this is why we only use one cup of hibiscus petals.]

=== Phase 2: Refrigeration

+ Stir white sugar into container until dissolved.

+ Add cold water to fill container.

+ Let sit in refrigerator for 8-12 hours.

+ Serve chilled.
